...
> 51个经过验证的可复用组件，按使用频率排序。直接复制代码使用。
...
| 场景 | 推荐组件 |
|------|------|
| Hero区/首屏大标题 | Section Header、Poster Split、Dark Typographic Ending、Serena Lab Hero |
| 图片展示/头像/IP | 头像集群、肖像分割引号、暗化肖像结尾、作品集图片组件 |
| 正文重点标注 | Key Insight、Pull Quote、手写批注、荧光笔高亮 |
| 卡片类 | 卡片组件库、三列Chair、编号卡片网格、悬停揭示卡片 |
| 引用/金句 | Key Insight、Pull Quote、巨大引号、极简留白引号、肖像分割引号 |
| 代码/终端 | macOS代码窗口、笔记本纸代码、打字机代码、极简白底代码 |
| 导航/切换 | 导航栏、Filter标签栏、Tab切换、Modal滚动阅读 |
| 步骤/流程 | 系统流程条、横向时间线、左右分栏问答、交互清单 |
| 对比/数据 | Do/Don't、对比表A、对比表B、日历网格 |
| 作品集/案例 | 点击弹窗详情、缩略图轨道+侧面板、悬停翻转卡片 |

选择原则：
- 连贯性优先。同一页面内同类内容尽量使用同一类组件变体。
- 内容决定形式。先判断内容是流程、对比、引用、代码、作品还是 CTA，再选组件。
- 动效和强视觉组件克制使用。每页最多 1-2 个视觉记忆点，避免做成组件展览。
...
虚线边框 + Schoolbell字体标签，适合提示/注意事项/笔记感。
...
.quote-highlight .quote-title {
  ...
  margin-bottom: 12px;
  display: inline;
  background: linear-gradient(to top, rgba(240,112,64,0.26) 45%, transparent 45%);
  line-height: 1.8;
  padding: 2px 4px;
}
...
.quote-minimal .quote-body .hl {
  background: linear-gradient(transparent 60%, rgba(240,112,64,0.20) 60%);
  padding: 0 4px;
}
...
虚线边框 + Schoolbell标签，手绘草图感，适合技能/工具/特性介绍。
...
Schoolbell字体标题 + 虚线边框，适合轻松/教程类内容。
...
不对称grid布局（左0.35fr 右1fr），左边放大标题区（sticky），右边每行是Spectral大号编号+两列对比值。暖色背景，无深色。
...
.mag-title {
  font-family: 'Playfair Display', 'Spectral', 'Noto Serif SC', serif;
