...
| 色块交替行 | 对比/表格 | 暖底行(#F9F8F6) vs 白底行(#fff) 交替 |
| 大箭头流程 | 步骤连接 | 主色或强调色箭头(48px)连接流程块，点缀色只用于一两个关键节点 |
| 代码面板 | 代码/文件树/命令 | 深色底+Fira Code+三色圆点title bar |
| 金句装饰 | 核心观点/结尾 | oversized引号+白底圆角卡片 |
| 问题→解法对比行 | before/after | 左强调色底右主色底，✗ vs ✓ 前缀 |
...
- 署名固定为：「Serena」
- 品牌三色比例：主色为主，强调色负责行动与信息，点缀色只做少量记忆点
- 背景主色：近白暖底 `#FFFDF9` / 纸感白交替底 `#F9F8F6`，深色面板用 `#1A1A2E`
...
- [ ] 是否有丑的默认组件（竖线列表、强调色圆点、默认blockquote）
