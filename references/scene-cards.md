...
### P1 封面
- 大标题（84px）用寒蝉活楷体（ChillHuoKai Con），关键词优先用珊瑚橙高亮块（`background: #F07040; color: #fff; padding: 4px 16px; border-radius: 6px`），天蓝只做少量点缀
- 副标题（44px）一行显示，紧跟标题下方，`white-space: nowrap`
- 圆形头像（`avatar.jpg`，120px，`border: 4px solid #F07040`）
- 署名「Serena」44px + 介绍34px
- 整体边框：`border: 28px solid #F07040`
...
### 最后一页 尾页
- 金句（oversized引号装饰 `"` ，Spectral 200px，opacity 0.15）
- 圆形头像 + 署名「Serena」
- CTA：「关注 Serena」
...
| 大箭头流程 | 步骤连接 | 珊瑚橙或纽约客红箭头(48px)连接流程块，天蓝只用于一两个关键节点 |
| 代码面板 | 代码/文件树/命令 | 深色底+Fira Code+三色圆点title bar |
| 金句装饰 | 核心观点/结尾 | oversized引号+白底圆角卡片 |
| 问题→解法对比行 | before/after | 左纽约客红底右珊瑚橙底，✗ vs ✓ 前缀 |
...
- [ ] 品牌三色比例 6:3:1（珊瑚橙/纽约客红/天蓝）
- [ ] 头像和署名是否正确（圆形头像+「Serena」）
