...
| **天蓝** | `#2B7FD8` | 强调色 | CTA、活跃状态、关键数据、链接、选中态、流程节点 |
| **纽约客红** | `#CC2936` | 点缀色 | badges、异常信号、少量高亮、装饰标记、强调性小面积提醒 |
...
- 底色：`#FFFDF9`（近白暖底，页面主底）、`#F9F8F6`（纸感白交替底，适合 section 交替区）、`#FFFFFF`（纯白组件底/局部交替）、`#FEF1EC`（由主橙低比例稀释的淡橙纸底，适合小面积试用）、`#FFF3EE`（浅橙米，较重要的次底）、`#F5FAFF`（浅蓝纸底，适合轻量引用/蓝色边框组件）
- 大面积页面底色优先使用 `#FFFDF9` 作为主底，配合 `#F9F8F6` 或 `#FFFFFF` 交替；`#FEF1EC` 和 `#FFF3EE` 只做局部氛围、重点次底或小面积暖感，不整页厚铺。
...
| 边框 | 大面积强调边框细于40px（统一40px，不得更细）；边框颜色必须来自品牌三色，优先用强调蓝或主橙 |
...
- **选中文本高亮**: `::selection { background: #2B7FD8; color: #fff; }`
...
- **数字/统计**: 优先用天蓝 `#2B7FD8`，异常/爆发信号可用纽约客红 `#CC2936`，不用纯黑
