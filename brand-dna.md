...
- 底色：`#F9F8F6`（纸感白主底）、`#FEF1EC`（由主橙低比例稀释的淡橙纸底，适合页面交替区试用）、`#FFFFFF`（纯白交替）、`#FFF3EE`（浅橙米，较重要的次底）、`#F5FAFF`（浅蓝纸底，适合轻量引用/蓝色边框组件）、`#FFFDF9`（近白暖底，适合轻量对话气泡）
- 大面积页面底色优先使用 `#F9F8F6` + `#FEF1EC` 或 `#FFFFFF` 交替；`#FFF3EE` 只做局部氛围、重点次底或小面积暖感，不整页厚铺。
